...
#-		-$t3	counter value
#-		-$t5	temp pointer to address we want to store at
#-		-$t4	temp value of $a0 we are shifting
#-		-$t0	temp character to output
#-#----------------------------------------

	li	$t3, 8		# counter for the storage of 8 hex digits
	la	$t5, table	# load the address of table in $t5
loop1:		
	srl	$t4, $a0, 28	# take high 4 bits and put it in the bottom of a temporary register
	add	$t4, $t4, $t5	# add to base of table
	#lb 	$t0, ($t4)	# get a character to output from $t4 into $t0
	sb	$t4, 0($a1)	# store what was in $t4 to address at $a1 which is table[count]
