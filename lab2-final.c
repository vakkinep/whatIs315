...
void extract_float(INTFLOAT_PTR xp, float f) {
   ...
   floatbits = (unsigned int) * (unsigned int *) &f; // extract binary float #

//   printf("BITS TO EXTRACT 0x%08X\n", floatbits); 
...
//   printf("FOR FLOAT VAL %f--------\n\tSIGN %i\n\tEXP %i\n\tFRACTION %08X\n",
//         f ,xp->sign, xp->exponent, xp->fraction);
}
...
void pack_float(float* f, INTFLOAT_PTR x) {
   ...
   complement |= (0x007FFFFF & ((x->fraction) >> 7) );// throwing away hidden 1
...
void normalize(INTFLOAT_PTR x) {
   ...
   printf("NORMALIZED FRACTION: %08X\n", x->fraction);
   printf("NORMALIZED EXPONENT: %08X\n\n", x->exponent);
}
